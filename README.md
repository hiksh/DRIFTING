...
| 분할 | 샘플 수 | 피처 수 | 타깃 소수 클래스 |
|---|---:|---:|---|
| Train (필터 후) | 1,592,468 | 70 | bot(1,360) / brute_force(1,062) / xss(436) |
| Test | 682,967 | 70 | — |

전처리: 분산=0인 피처 8개 제거 → StandardScaler. 범주형 피처 없음. 클래스 내 추가 zero-var 피처 20개 존재.

#### UNSW-NB15

| 분할 | 샘플 수 | 피처 수 | 타깃 소수 클래스 |
|---|---:|---:|---|
| Train (필터 후) | 59,009 | 176 | Backdoor(1,746) / Shellcode(1,133) / Worms(130) |
| Test | 38,005 | 176 | — |

전처리: 수치형 32개 StandardScaler + 범주형 2개(`protocol` 133→132 OHE, `service` 13→11 OHE) → D=176. Normal 클래스 56,000개 전량 사용 (benign cap 불필요).
